<template>
  ...
  <label>TotalJan2024 Tabela 4: {{ totalJan2024Tabela4 }}</label>
</template>
...
const dadosGrupos = computed(() => {
  // Protege contra undefined
  return store.tabela4?.grupos?.[0]?.subgrupos?.[0]?.items ?? []
})


// Computed para o total de jan_2024 dos itens de dadosGrupos
const totalJan2024Tabela4 = computed(() =>
  dadosGrupos.value.reduce((soma, item) => soma + (Number(item.jan_2024) || 0), 0)
)
...
function update(i) {
  if (props.tabelaKey && props.editable) {
    store.setTabela1(JSON.parse(JSON.stringify(rows.value)))
    markEdited(i)
  }
}
